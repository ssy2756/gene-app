...
If grouping by tier (either as an object keyed by tier name, or by including a "tier" field on each item), the tiers MUST be emitted in this exact order: "Essential" first, then "Advised", then "Optional" — this is the order the app displays them in, and it must match the order they appear on the page, which is always Essential, then Advised, then Optional. Do not emit them in a different order (e.g. Advised before Essential) even if that happens to be more convenient — always Essential → Advised → Optional.
