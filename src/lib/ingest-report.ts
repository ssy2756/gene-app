...
The document has a page titled (or similarly titled) "Tailored Fitness: Musculoskeletal Resilience for Every Step", which contains an "Exercise" subsection — this is the source for "metabolism" (despite the field's name, it holds the fitness/exercise recommendations shown in the app's "Fitness & exercise" list, not metabolic-pathway data). Scrape every individual recommendation/area shown under that Exercise subsection into "metabolism" as an array of objects, one per recommendation area, each with its own recommendation text (and a "type" label if the subsection groups them into named areas). Do not summarize the Exercise subsection into one or two generic bullets — extract every distinct recommendation it lists.
