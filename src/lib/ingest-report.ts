...
The document has a page titled (or similarly titled) "Tailored Fitness: Musculoskeletal Resilience for Every Step". This page contains MULTIPLE subsections, commonly including "Medical Recommendations", "Diet and Nutrition", and "Exercise". The schema field "exercise" must be scraped ONLY from the "Exercise" subsection of THIS "Tailored Fitness" page — not from "Medical Recommendations" (that belongs in medical_recommendations/condition_risk_overview instead), not from "Diet and Nutrition" (that belongs in diet_plan instead), and not from the document's separate "Your Metabolism" section (e.g. covering "Carbohydrate Intolerance", "Lactose Intolerance", metabolic-pathway/enzyme content — that data has no home in this schema and should simply be left out). Pulling content from any of those other sections into "exercise" is a mistake, even if it appears on the same page or under a similarly-named heading.
