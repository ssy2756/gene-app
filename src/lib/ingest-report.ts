...
The document has a page titled (or similarly titled) "Tailored Fitness: Musculoskeletal Resilience for Every Step". This page contains MULTIPLE subsections, commonly including "Medical Recommendations", "Diet and Nutrition", and "Exercise" — "metabolism" (despite the field's name, it holds the fitness/exercise recommendations shown in the app's "Fitness & exercise" list, not metabolic-pathway data) must be scraped ONLY from the "Exercise" subsection of that page. Do NOT pull content from that same page's "Medical Recommendations" subsection (that belongs in medical_recommendations/condition_risk_overview instead) or its "Diet and Nutrition" subsection (that belongs in diet_plan instead) into "metabolism" — including any of that other-subsection content here is a mistake, even if it appears on the same page right next to the Exercise subsection. Scrape every individual recommendation/area shown under the Exercise subsection specifically into "metabolism" as an array of objects, one per recommendation area, each with its own recommendation text (and a "type" label if the subsection groups them into named areas). Do not summarize the Exercise subsection into one or two generic bullets — extract every distinct recommendation it lists, but nothing from the page's other subsections.
