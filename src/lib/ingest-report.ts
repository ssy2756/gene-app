...
EXACT LAYOUT PATTERN — read this carefully, this is exactly how the page is drawn: this page shows a human body silhouette down the left side. Three colored horizontal connector lines run from the body out to the right, each ending near a tier label ("Essential", "Advised", "Optional", top to bottom in that order, each in its own color). The vitamin/mineral names and doses for a given tier are listed in a vertical column positioned BELOW that tier's own connector line/label and ABOVE the next tier's connector line/label — NOT below every label equally, and NOT in reading order from top of page to bottom regardless of section. Concretely: if "Essential"'s connector line has no list beneath it before "Advised"'s line begins, "Essential" has zero items — do not take items that are visually positioned under "Advised" or "Optional" and assign them to "Essential" just because "Essential" is the topmost label. Trace each item's vertical position on the page to the nearest tier label ABOVE it (between that label and the next one down), not to whichever label happens to be first. Do not assign tiers by list order/position-in-your-output (e.g. "first N items = Essential") — assign strictly by the item's own physical position on the page relative to the three connector lines.
