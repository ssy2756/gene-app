...
3. LIFESTYLE SECTIONS ARE OFTEN UNDER-EXTRACTED — BE THOROUGH. "vitamins_and_minerals" and "fitness_and_nutrigenomics" each have dedicated pages with specific, extractable content — they are not optional or summary sections. For vitamins_and_minerals, produce an array of objects, each with: "name" (the vitamin/mineral/nutrient name), "tier" (however the document groups/tiers it — e.g. Essential/Advised/Optional, or omit if the document doesn't tier them), "why" (the genetic reason given), and "dose" (the recommended dose/amount shown). For fitness_and_nutrigenomics, populate: "food_sensitivity" as an array of objects with "name" (the food/substance), "gene" (the genetic basis shown), and "level" (the sensitivity/tolerance level shown, e.g. Low/Moderate/High or the exact wording used); "musculoskeletal" as an object with "profile" (the fitness/muscle-type profile headline) and "detail" (the supporting genetic detail, e.g. gene names); and "metabolism" as an array of objects each with "recommendation" (a specific fitness/exercise recommendation sentence from the page). If any of these pages exist in the document, this data MUST be present in your output — an empty array or missing field here is only acceptable if that section is genuinely absent from the PDF.
